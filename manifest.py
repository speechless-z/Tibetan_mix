...
#python $scripts/manifest.py $data_root --ext wav --dest $data_deal/temp/ando --valid-percent 0.05 --test-percent 0.10
...
def main(args):
    ...
    # args.ext = wav,**/*表示匹配零个或多个子目录，即搜索整个目录树
    ...
    with open(os.path.join(args.dest, "train.tsv"), "w") as train_f:
        ...
        for fname in glob.iglob(search_path, recursive=True):
            ...
            if rand.random() <= args.valid_percent:
                dest = valid_f
            elif args.valid_percent < rand.random() <= (args.valid_percent + args.test_percent):
                dest = test_f
            else:
                dest = train_f
